...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
  ...
  <h1>${board.name}</h1>
   ...
   <br/> <br/>
   <!-- For Isotope -->
   <div id="linkcontainer " class="isotope clickable variable-sizes">
     <c:forEach items="${links}" var="link" varStatus="row">
	  	<!-- 
	  		<div class="item element transition"><a href" href="${link.url}" target="window"><i class="icon-heart icon-white"></i> ${link.name}</a></div>
	 	-->
	 	  <div class="box">
                <h2 class="box-title"><a href" href="${link.url}" target="window">${link.name}</a></h2>
                <div class="box-text">
                    ${link.description}
                </div>
            </div>
